...
maxlen = 128
batch_size = 64
config_path = '/home/tsing05/bojone/model/chinese_L-12_H-768_A-12/bert_config.json'
checkpoint_path = '/home/tsing05/bojone/model/chinese_L-12_H-768_A-12/bert_model.ckpt'
dict_path = '/home/tsing05/bojone/model/chinese_L-12_H-768_A-12/vocab.txt'
...
# 加载数据集
train_data = load_data('/home/tsing05/bojone/data2/train.json')
#train_data = train_data[:10000]
valid_data = load_data('/home/tsing05/bojone/data2/dev.json')
...
class data_generator(DataGenerator):
    ...
    def __iter__(self, random=False):
        ...
        for is_end, d in self.sample(random):
            ...
            #log.info(json.dumps(d).decode("unicode-escape"))
            # 整理三元组 {s: [(o, p)]}
            spoes = {}
...
def extrac_subject(inputs):
    ...
    print("9090909009")
    print K.int_shape(subject_ids) 
    start = batch_gather(output, subject_ids[:, :1])
    print K.int_shape(start) 
    end = batch_gather(output, subject_ids[:, 1:])
    subject = K.concatenate([start, end], 2)
    print  K.int_shape(subject) 
    return subject[:, 0]
...
def extract_spoes(text):
    ...
    #mapping = [x for x in mapping if x != []]
    #print "opopoppopopopopopopopo"
    case_list_righ = str(tokens).replace('u\'','\'')
